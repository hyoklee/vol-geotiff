...
NC_interpret_magic_number(char* magic, NCmodel* model)
{
    ...
    /* TIFF magic: little-endian (II*\0) or big-endian (MM\0*) - route to HDF5 VOL */
    if((magic[0]=='I' && magic[1]=='I' && magic[2]=='\x2a' && magic[3]=='\x00') ||
       (magic[0]=='M' && magic[1]=='M' && magic[2]=='\x00' && magic[3]=='\x2a')) {
	model->impl = NC_FORMATX_NC4;
	model->format = NC_FORMAT_NETCDF4;
	goto done;
    }
